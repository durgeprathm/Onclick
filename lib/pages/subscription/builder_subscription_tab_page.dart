...
class BuilderSubscriptionTab extends StatefulWidget {
  @override
  _BuilderSubscriptionTabState createState() => _BuilderSubscriptionTabState();
}

class _BuilderSubscriptionTabState extends State<BuilderSubscriptionTab> {
  ...
  getSPdata() async {
    ...
    if (resid == 200) {
      int rowcount = response["rowcountbuilder"];
      if (rowcount > 0) {
        var providersd = response["buildersubscriptionlist"];
  ...
  ListTile makeListTile(Subscriptions subscription) => ListTile(
        ...
        trailing: Column(
          children: [
            ...
            Expanded(
              child: subscription.subscribeStatus.isEmpty
                  ? FlatButton(
                      ...
                      onPressed: () async {
                        var uName = await SharedPreferencesConstants.instance
                            .getStringValue(
                                SharedPreferencesConstants.USERFULLNAME);
                        var uEmail = await SharedPreferencesConstants.instance
                            .getStringValue(
                                SharedPreferencesConstants.USEREMAILID);
                        var uMobNo = await SharedPreferencesConstants.instance
                            .getStringValue(
                                SharedPreferencesConstants.USERMOBNO);
                  ...
                  : Text(subscription.subscribeStatus,
                      style: subStatusheadingStyle),
            ),
          ],
        ),
        onTap: () {
          Navigator.of(context).push(
            MaterialPageRoute(builder: (_) {
              return SubscriptionDetailPage(subscription, 'builder');
            }),
          );
        },
      );
